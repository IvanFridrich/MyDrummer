...
int32_t apply_end_ramp(int32_t voiced, uint32_t position, uint32_t length)
{
    ...
    const uint32_t start =
        (length > VOICE_END_RAMP_SAMPLES) ? length - (uint32_t)VOICE_END_RAMP_SAMPLES : 0;
    if (position >= start)
    {
...
#define R1000(x)                                                                                   \
    R100(x), R100(x), R100(x), R100(x), R100(x), R100(x), R100(x), R100(x), R100(x), R100(x)
...
void test_single_voice_plays_through()
{
    ...
    for (int i = 0; i < 4; ++i)
        TEST_ASSERT_EQUAL_INT16(0, tail[i]);
...
void test_two_voices_sum()
{
    ...
    for (uint32_t i = 0; i < 8; ++i)
    {
        const int32_t v1 = apply_end_ramp(mul_q15(kRamp[i], (int16_t)MASTER_VOLUME_Q15), i, 8);
        const int32_t v2 = apply_end_ramp(mul_q15(kSteady[i], (int16_t)MASTER_VOLUME_Q15), i, 8);
...
void test_saturating_clip_on_loud_sum()
{
    ...
    {
        const int32_t v1 = apply_end_ramp(mul_q15(kLoud[i], (int16_t)MASTER_VOLUME_Q15), i, 4);
        const int32_t v2 = apply_end_ramp(mul_q15(kLoud[i], (int16_t)MASTER_VOLUME_Q15), i, 4);
        TEST_ASSERT_EQUAL_INT16(sat_i16(v1 + v2), buf[i]);
    }
}
...
void test_retrigger_fade_decays_old_voice_gain()
{
    ...
    TEST_ASSERT_TRUE_MESSAGE(pool.voices()[0].gain_q15 < initial_gain,
...
void test_retrigger_fade_eventually_deactivates_old_voice()
{
    ...
    for (int i = 0; i < 20; ++i)
        mx.get_samples(buf, AUDIO_CHUNK);
