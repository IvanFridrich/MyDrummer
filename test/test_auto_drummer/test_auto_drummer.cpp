...
void test_funk_loop_repeats_exactly()
{
    ...
    for (int i = 0; i < 6615; ++i)
        count1 += ad.tick(64, trig, tvel, 256);
    for (int i = 0; i < 6615; ++i)
        count2 += ad.tick(64, trig, tvel, 256);
...
void test_humanize_event_count_has_no_systematic_drift()
{
    ...
    select_funk(off);
    off.next_speed();
    off.set_humanize(false);
    select_funk(on);
    on.next_speed();
    on.set_humanize(true);

    static uint8_t tr[256], tv[256];
    for (int i = 0; i < 1654; ++i)
    {
        off.tick(64, tr, tv, 256);
        on.tick(64, tr, tv, 256);
    }
    ...
    TEST_ASSERT_LESS_OR_EQUAL_MESSAGE(
        10, slop, "humanized event count must not drift from exact (no drop/dup)");
}
...
void test_all_styles_fire_events()
{
    ...
    for (int s = 1; s < static_cast<int>(AutoStyle::COUNT); ++s)
    {
        ...
        for (int i = 0; i < 6000; ++i)
            total += do_tick(ad);
        TEST_ASSERT_GREATER_THAN_MESSAGE(0, total, "every style must fire events in the intro");
    }
}
