...
uint8_t AutoDrummer::gm_to_sample_id(uint8_t gm_note)
{
    ...
    {
    case 36:
        return static_cast<uint8_t>(SampleId::SAMPLE_KICK);
    case 38:
    case 40:
        return static_cast<uint8_t>(SampleId::SAMPLE_SNARE);
    case 50:
    case 48:
        return static_cast<uint8_t>(SampleId::SAMPLE_TOM_HI);
    case 47:
    case 43:
        return static_cast<uint8_t>(SampleId::SAMPLE_TOM_MID);
    case 45:
    case 41:
        return static_cast<uint8_t>(SampleId::SAMPLE_TOM_LO);
    case 42:
        return static_cast<uint8_t>(SampleId::SAMPLE_HIHAT_CLOSED);
    case 46:
        return static_cast<uint8_t>(SampleId::SAMPLE_HIHAT_OPEN);
    case 44:
        return static_cast<uint8_t>(SampleId::SAMPLE_HIHAT_PEDAL);
    case 49:
    case 57:
        return static_cast<uint8_t>(SampleId::SAMPLE_CRASH);
    case 51:
    case 53:
    case 59:
        return static_cast<uint8_t>(SampleId::SAMPLE_RIDE);
    case 31:
        return static_cast<uint8_t>(SampleId::SAMPLE_STICKS);
    case 56:
        return static_cast<uint8_t>(SampleId::SAMPLE_COWBELL);
    case 54:
        return static_cast<uint8_t>(SampleId::SAMPLE_TAMBOURINE);
    default:
        return 0xFF;
    }
}
...
AutoDrummer::AutoDrummer()
    : style_(AutoStyle::Off), speed_idx_(0) // slow speed by default
      ,
      intro_count_(0), loop_count_(0), loop_start_sample_(0),
      loop_length_samples_(1) // non-zero guard
      ,
      pos_samples_(0), intro_event_idx_(0), in_loop_(false), loop_pos_samples_(0),
      loop_event_idx_(0), humanizer_()
{
...
void AutoDrummer::load_pattern()
{
    ...
    const float spt =
        static_cast<float>(SAMPLE_RATE) * 60.0f / static_cast<float>(AUTO_DRUMMER_PPQN * bpm);

    loop_start_sample_ = static_cast<uint32_t>(pat.loop_start_tick * spt + 0.5f);
    loop_length_samples_ =
        static_cast<uint32_t>((pat.loop_end_tick - pat.loop_start_tick) * spt + 0.5f);
    if (loop_length_samples_ == 0)
        loop_length_samples_ = 1;
...
{
    const uint8_t next =
        (static_cast<uint8_t>(style_) + 1u) % static_cast<uint8_t>(AutoStyle::COUNT);
